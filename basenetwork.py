...
class Network:
    ...
    @classmethod
    def base_model(cls,
                   num_inputs=4, hidden_layers=[20, 20, 20, 20],
                   activation_layers='tanh', num_outputs=1,
                   ub=np.array([20, 0.18, 0.4, 0.15]),
                   lb=np.array([0, 0.08, 0.1, 0.05])):
        ...
        for layer_size in hidden_layers:
            x = tf.keras.layers.Dense(
                layer_size, activation=activation_layers)(x)
